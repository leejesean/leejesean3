    ...
    for monster in team:
        if collide(boy,monster):#몬스터랑 캐릭터 부딪쳤을때!
            ...
            if monster.count>5:
                monster.Damage=FALSE
                monster.Daed=TRUE
            if monster.count==1 or monster.count==2 or monster.count==3 or monster.count==4or monster.count==5:#맞고 다시 원상복귀할때
                monster.count3=0
            if monster.count==6:#네번째일때는 죽어야하므로 두번째 카운트를초기화해준다
                monster.count2=0
            ...
            if monster.count==1 or monster.count==2 or monster.count==3 or monster.count==4:#맞고 다시 원상복귀할때
                monster.count3=0
            if monster.count==5:#네번째일때는 죽어야하므로 두번째 카운트를초기화해준다
                monster.count2=0
        ...
        if monster.Damage==TRUE and monster.count3>2:
            monster.Damage=FALSE
            monster.Daed=FALSE
        elif monster.Daed==TRUE and monster.count2>3:
            team.remove(monster)
