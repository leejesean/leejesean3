...
class Monster:
    ...
    def update(self):
        ...
        elif self.turn == 1 and self.Damage==FALSE and self.Daed==FALSE and 200>self.hp:#마지막 피200남았을때
            self.movey=8*380
            self.monstery=245
            self.frame = (self.frame + 1 ) % 4
        elif self.turn == -1 and self.Damage==FALSE and self.Daed==FALSE and 200>self.hp:#마지막 피200남았을때
...
class Smallball:
    ...
    def update(self):
        ...
        if self.x>750 or self.x<50:
            self.x=xx
    def draw(self):
        self.image.clip_draw(self.frame*40, self.movey*30, 40, 30,self.x,90)
    def get_bb(self):
