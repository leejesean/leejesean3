...
class Monster:
    def __init__(self):
        self.frame = 0
        self.monsterx,self.monstery=random.randint(200,700),140
        ...
        self.damage_sound = load_wav('몬스터2데미지.wav')
    ...
    def draw(self):
        ...
        self.image2.clip_draw(0, 0, 30, 5,self.monsterx, self.monstery-80)
        self.image3.clip_draw(0, 0, self.hp, 5,self.monsterx, self.monstery-80)
    ...
    def get_bb(self):
        return self.monsterx-50,self.monstery-70,self.monsterx+50,self.monstery+70
    def draw_bb(self):
        draw_rectangle(*self.get_bb())
class Ball:
    def __init__(self):
...
################################실행################################
    if  state==RIGHT_RUN and bkcheck == FALSE :
        ...
        bkx=bkx-17.5
        monster.monsterx-=100
        print(monster.monsterx)
        movey=760
        magicx=0#여기버그
    ...
    for monster in team:
        if collide(boy,monster):#몬스터랑 캐릭터 부딪쳤을때!
            ...
            if monster.hp<0:#3번맞으면 죽게된다
                monster.Damage=FALSE
                monster.Daed=TRUE
            if monster.count<12:#맞고 다시 원상복귀할때
                monster.count3=0
            if monster.count>12.4:#네번째일때는 죽어야하므로 두번째 카운트를초기화해준다
                monster.count2=0
