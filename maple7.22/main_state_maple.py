...
class Grass:
    def __init__(self):
        ...
        self.bgm=load_music('배경음악마을.mp3')
...
class Boy:
    ...
    def update(self):
        if self.isGoRight==TRUE or self.isGoLeft==TRUE:
            self.frame = (self.frame + 1) % 4
...
#-----------------------------------------------------------------------------------------------------------------------
def enter():
    global boy,grass,magic,magic2,magic3,npc,npc2,npc3,ball,balls
    boy=Boy()
    npc=Npc()
    npc2=Npc2()
    npc3=Npc3()
...
##################################실행##############################
        ...
        if state==RIGHT_STAND:
            ...
            movey=955
        elif state==RIGHT_JUMP and right == TRUE:
           if isJump == FALSE:
                movey=475
                jump = 35.0
                gravity = 7.0
                isJump = TRUE
        elif state==LEFT_JUMP and right == FALSE:
           if isJump == FALSE:
                movey=570
                jump = 35.0
                gravity = 7.0
