...
class Monster:
    def __init__(self):
        self.frame = 0
        self.monsterx,self.monstery=random.randint(200,800),245
        self.speed = 10
        self.hp=700
        self.turn = 1
        self.Daed=0
    ...
    def update(self):
        ...
        if self.turn == 1 and self.Damage==FALSE and self.Daed==FALSE and self.hp>600:#오른쪽걸어갈때
            ...
            self.monstery=245
            self.frame = (self.frame + 1 ) % 4
        elif self.turn == 1 and self.Damage==FALSE and self.Daed==FALSE and self.hp>=400 and self.hp<=600 and 0<self.hp and self.hp<=200:#오른쪽날아갈때
            self.movey=4*380
            self.monstery=370
            self.frame = (self.frame + 1 ) % 1
        elif self.turn == -1 and self.Damage==FALSE and self.Daed==FALSE and self.hp>=400 and self.hp<=600 and 0<self.hp and self.hp<=200:#왼쪽날아갈때
            self.movey=5*380
            self.monstery=370
            self.frame = (self.frame + 1 ) % 1
        elif self.turn == 1 and self.Damage==FALSE and self.Daed==FALSE and 200<self.hp and self.hp<400:#오른쪽날아갈때
            self.movey=2*380
            self.monstery=245
            self.frame = (self.frame + 1 ) % 19
        elif self.turn == -1 and self.Damage==FALSE and self.Daed==FALSE and 200<self.hp and self.hp<400:#왼쪽날아갈때
            self.movey=3*380
            self.monstery=245
            self.frame = (self.frame + 1 ) % 19
            ...
            self.monstery=245
            self.frame = (self.frame + 1 ) % 12
        if self.monsterx >740:
            self.turn *=-1
        elif self.monsterx < 60:
            self.turn *=-1
    ...
    def get_bb(self):
        return self.monsterx-50,self.monstery-120,self.monsterx+50,self.monstery+70
    def draw_bb(self):
        draw_rectangle(*self.get_bb())
class Ball:
    def __init__(self):
...
class Smallball:
    ...
    global tt
    def __init__(self):
        self.x,self.y =xx,200
        self.movey=0
        self.frame=0
        self.fall_speed = random.randint(40,30)
    ...
    def draw(self):
        self.image.clip_draw(self.frame*40, self.movey*30, 40, 30,self.x,200)
    def get_bb(self):
...
#------------------------------------------------------------------------------
def enter():
    global boy,boydead, grass,magic,magic2,magic3,monster,team,ball,balls,bigball,bigballs
    boy=Boy()
    boydead=Boydead()
    grass = Grass()
    magic=Magic()
