...
class Monster:
    def __init__(self):
        self.frame = 0
        self.monsterx,self.monstery=random.randint(200,800),140
    ...
    def update(self):
        ...
        elif self.turn == -1 and self.Damage==FALSE and self.Daed==FALSE:
            self.movey=8*160
            self.frame = (self.frame + 1 ) % 3
    ...
    def draw(self):
        self.image.clip_draw(self.frame * 170,  self.movey, 170, 160,self.monsterx, self.monstery)
    def get_bb(self):
        return self.monsterx-50,self.monstery-50,self.monsterx+50,self.monstery+50
    def draw_bb(self):
        draw_rectangle(*self.get_bb())
...
#------------------------------------키조작-----------------------------------
            ...
            if event.key==SDLK_ESCAPE:
               game_framework_maple.quit()
                ...
                game_framework_maple.change_state(main_state_maple4)#chage는 exit를실행!
...
def update():
    ...
    for monster in team:
        if collide(boy,monster):#몬스터랑 캐릭터 부딪쳤을때!
            ...
            monster.Damage=TRUE
            monster.count=monster.count+0.25#맞았을때+1
            print(monster.count)
            if monster.count>4:#3번맞으면 죽게된다
                monster.Damage=FALSE
                monster.Daed=TRUE
            if monster.count==1 or monster.count==2 or monster.count==3 or monster.count==4:#맞고 다시 달리게
                monster.count3=0
            if monster.count>4.25:#다섯번째일때는 죽음
                monster.count2=0

        elif collide2(magic2,monster):
            monster.Damage=TRUE
            monster.count=monster.count+0.25#맞았을때+0.5
            print(monster.count)
            if monster.count>4:
                monster.Damage=FALSE
                monster.Daed=TRUE
            if monster.count==1 or monster.count==2 or monster.count==3or monster.count==4:#맞고 다시 원상복귀할때
                monster.count3=0
            if monster.count>4.25:#네번째일때는 죽어야하므로 두번째 카운트를초기화해준다
                monster.count2=0
            ...
            if monster.count>8:#3번맞으면 죽게된다
                monster.Damage=FALSE
                monster.Daed=TRUE
            if monster.count==1 or monster.count==2 or monster.count==3 or monster.count==4or monster.count==5or monster.count==6or monster.count==7or monster.count==8:#맞고 다시 원상복귀할때
                monster.count3=0
            if monster.count>8.25:#네번째일때는 죽어야하므로 두번째 카운트를초기화해준다
                monster.count2=0

        #죽을때!---
        if monster.Damage==TRUE and monster.count3>2:
            monster.Damage=FALSE
            monster.Daed=FALSE
        elif monster.Daed==TRUE and monster.count2>2:
            team.remove(monster)
